...
{
    public class RestfulDeleteHandlerActionSource : IActionSource
    {
        private static readonly string _methodName =
            ReflectionHelper.GetMethod<RestfulDeleteHandler<string>>(x => x.Delete(null)).Name;

        public IEnumerable<ActionCall> FindActions(TypePool types)
        {
            return from entities in types.TypesMatching(t =>t.CanBeCastTo<AwesomeEntity>()) 
                   where entities.IsConcrete()
                   select typeof (RestfulDeleteHandler<>).MakeGenericType(entities) 
                   into handlerType 
